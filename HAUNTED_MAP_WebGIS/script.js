...
// ══════════════════════════════════════════════════
//  AUDIO — File MP3 + Toggle + Auto Loop
// ══════════════════════════════════════════════════

const bgMusic = new Audio();
bgMusic.loop   = true;   // ← otomatis ulang kalau habis
bgMusic.volume = 0.35;

// ── Ganti URL ini dengan link MP3 horor kamu ──
// Bisa dari Google Drive, GitHub raw, atau file lokal
const MUSIC_URL = 'horror-ambient.mp3'; // ← ganti ini

bgMusic.src = MUSIC_URL;

// Handle kalau file tidak ditemukan
bgMusic.addEventListener('error', () => {
  console.warn('File audio tidak ditemukan, pakai Web Audio fallback.');
  useFallbackAudio();
});
...
function toggleAudio() {
  if (bgMusic.paused) {
    bgMusic.play().catch(() => {}); // catch autoplay policy
...
// ── Auto-play saat pertama klik (browser policy) ──
...
// ── Fallback Web Audio kalau file tidak ada ──
...
function playDeepDrone() {
  [36.7, 38.9, 55].forEach((freq, i) => {
    ...
    osc.connect(ws); ws.connect(g); g.connect(gainNode);
    osc.start(); lfo.start();
    oscNodes.push(osc, lfo);
  });
}

function playWindLayer() {
  ...
  const wg = audioCtx.createGain(); wg.gain.value = 0.055;
  const wlfo = audioCtx.createOscillator();
  const wlg  = audioCtx.createGain();
  wlfo.frequency.value = 0.07; wlg.gain.value = 0.04;
  wlfo.connect(wlg); wlg.connect(wg.gain);
  noise.connect(bp); bp.connect(wg); wg.connect(gainNode);
  noise.start(); wlfo.start();
  oscNodes.push(noise, wlfo);
}

function scheduleRandomWhisper() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    data[i] = (Math.random() * 2 - 1) * Math.sin(Math.PI * t / dur) * 0.6;
  }
  const src = audioCtx.createBufferSource();
  src.buffer = buf;
  const f = audioCtx.createBiquadFilter();
  f.type = 'bandpass'; f.frequency.value = 1800 + Math.random() * 600; f.Q.value = 2.5;
  const g = audioCtx.createGain(); g.gain.value = 0.08 + Math.random() * 0.06;
  src.connect(f); f.connect(g); g.connect(audioCtx.destination);
  src.start();
  setTimeout(() => scheduleRandomWhisper(), 6000 + Math.random() * 14000);
}
...
function scheduleCreak() {
  ...
  for (let i = 0; i < data.length; i++) {
    const t = i / audioCtx.sampleRate;
    data[i] = Math.sin(2 * Math.PI * (200 + t * 400) * t) * Math.exp(-t * 1.5) * (Math.random() * 0.3 + 0.7);
  }
  const src = audioCtx.createBufferSource();
  src.buffer = buf;
  const g = audioCtx.createGain(); g.gain.value = 0.12 + Math.random() * 0.1;
  src.connect(g); g.connect(audioCtx.destination);
  src.start();
  setTimeout(() => scheduleCreak(), 10000 + Math.random() * 20000);
}
